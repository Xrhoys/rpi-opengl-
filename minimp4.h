...
#define MALLOC(t, p, size) p = (t)malloc(size); if (!(p)) { NERROR("out of memory"); }
...
broken_android_meta_hack:
            ...
            {
                NERROR("invalid box size (broken file?)");
            }
            ...
            if (box_bytes == 1)           // 64-bit sizes
            {
                ...
                if (box_bytes)
                {
                    NERROR("UNSUPPORTED FEATURE: MP4BoxHeader(): 64-bit boxes not supported!");
                }
                box_bytes = minimp4_read(mp4, 4, &eof_flag);
#endif
                if (box_bytes < 16)
            ...
            for (i = 0; i < NELEM(g_fullbox); i++)
            {
                if (box_name == g_fullbox[i].name)
                {
                    ...
                    {
                        NERROR("unsupported box version!");
                    }
        ...
        case BOX_co64:
            ...
            for (i = 0; i < tr->chunk_count; i++)
            {
                ...
                if (box_name == BOX_co64)
                {
                    ...
                    {
                        NERROR("UNSUPPORTED FEATURE: 64-bit chunk_offset not supported!");
                    }
#endif
